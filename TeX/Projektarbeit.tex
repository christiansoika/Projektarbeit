...
\end{figure}
This paper deals with the effects of particles on the conditions of isotropic turbulent flows.
There are many examples of these particle-laden turbulent flows in nature, for example in volcanic eruptions and the "white water" of breaking waves.
\newline
Multiphase flows also occur in technical applications. Spray automization in fuel injectors, cyclonic particle separation in oil rafineries and accumulation of sediment in pipelines are just three of many cases, where predictions of the influence of particles on the turbulence are of huge interest.
\newline
Another major industrial interest is the fuel dispersion and combustion of dispersed fuel in a combustion chamber. To get a homogeneous and fast combustion, both should take place at turbulent flow conditions. So it is an important question, in which way and intensity the particles have an impact on the turbulence.
\newline
Another important example is an fluidized bed. This is an application to realize an effective suspension and combustion of fluid particles in a vessel. The particles, which are at the bottom of the vessel, are suspended by a fluid, in most cases a gas, that streams from beyond the particles through the porous bottom, if the pressure is larger than the gravity of the particles. While this happens the fluid suspends the particles and the contact between fluid and particles is very intimate, which fascilates for example the combustion of material with a low caloric content, the in situ absorption of the pollutants deriving from the combustion or the action of a catalyst. This process aims to produce a suspension with more homogeneous regions. Hence turbulence is a very important requirement to produce a homogeneous fluid stream for an effective and complete combustion.
\newline
In this work the influence of the particles on the streaming conditions is presented by using DNS and LES simulations. The focus lies on analyzing the energy transfer between particles and fluid as well as the intensity of  decrease of turbulent kinetic energy in the fluidial phase.  
At the beginning some mathematical background information about describing turbulent single-phase and particle-laden flows are given. 
Here the Navier-Stokes equations and the Kolmogorov scales are presented as an approach to model turbulence.
\newline
Expanding this set of equations for particle laden flows a new term is introduced to the equations of motion which describes the forces acting between fluid and particles. 
Also the coupling rate, that describes the exchange in energy between particles and fluid is introduced.
\newline
Then the computational basics of methods used in this work are introduced. First the simulation methods DNS and LES, how they work and in which case what one is the most fitting is explained.
Then the used discretization tool for integration of the particle tracking equations, working with a predictor-corrector-scheme, is described.     
Finally the computational method of 'particle clustering' is introduced, aiming to decrease the computational effort of particle-laden simulations. It works in the way that a specific amount of particles are coupled to a cluster and each cluster is considered numerically as one particle.
\newline
In the following the deviations caused by using this method are determined and the applicability is evaluated. Therefore the influence of the number of particles per cluster on the accuracy of the simulations of the turbulent kinetic energy and the exchanged energy between particles and fluid per time is identified.
...
\nomenclature{\vec{g}}{Gravity konstant}
\end{equation}

...
The delta function results in one if $\vec{x}-\vec{x}_\mathrm{p}^\mathrm{n}$ becomes zero and otherwise it is zero. By this means, the coupling forces act at the position of each particle on the fluid. 
...
\begin{equation}
\rho_\mathrm{p}\frac{d\vec{v}_\mathrm{p}^\mathrm{n}}{dt} = \rho_\mathrm{p}\vec{g}+\vec{f}_\mathrm{d}^\mathrm{n}
\end{equation}
...
\end{equation} it follows the equation for the relaxation time
\begin{equation}
...
With equation (\ref{C_d}) and the Reynolds number in relation to the particles $Re_\mathrm{p}^\mathrm{n}=\frac{v_\mathrm{r}^\mathrm{n}d}{\nu}$, with the particle diameter d it follows
\begin{equation}
...
\end{equation}
with $f_\mathrm{d} = 1+0.15Re_\mathrm{p}^\mathrm{n^{0.687}}$, which is a factor describing the flow conditions and not the drag force mentioned before.
\newline

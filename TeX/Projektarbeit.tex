...
{\large 8. Januar 2018} \\[1.5cm] % heutiges Datum
...
\nomenclature[rU1]{$\bar{U}(\vec{x})$}{filtered velocity field}
\nomenclature[re3]{$\vec{e}$}{unit vector in the same direction as $\vec{u}'$}
...
\nomenclature[gZp]{$\vec{\Omega}_\mathrm{p}$}{vorticity vector of the undisturbed flow around one particle}
\nomenclature[gzp]{$\vec{\omega}_\mathrm{p}$}{particle angular velocity vector}
...
\nomenclature[ru2]{$\vec{u}$}{fluid velocity vector}
...
\nomenclature[rRex]{$Re_\lambda$}{Taylor-scale Reynolds number}
\nomenclature[rRel]{$Re_\mathrm{L}$}{Reynolds number for the integral scale}
\nomenclature[gt]{$\matrix{\bar{\tau}}$}{stress tensor}
\nomenclature[rq]{$\vec{q}$}{heat conduction vector}
\nomenclature[gc]{$\gamma$}{isentropic exponent}
\nomenclature[re4]{$e$}{specific internal energy}
...
\nomenclature[rn1]{$N_\mathrm{p}$}{number of physical point particles}
\nomenclature[rn0]{$N_\mathrm{c}$}{number of computational point particles}
\nomenclature[gY]{$\Psi$}{integral coupling rate}
\nomenclature[gYp]{$\Psi_\mathrm{p}$}{coupling rate for one particle}
\nomenclature[gYpp]{$\Psi_\mathrm{pp}$}{integral coupling rate using the point-particle approach}
\nomenclature[gtp]{$\tau_\mathrm{p}$}{particle relaxation time}
\nomenclature[gD]{$\Delta$}{cell length}
\nomenclature[rdi]{$d_i$}{distance between particle position and cell center}
\nomenclature[gWv]{$\phi_\mathrm{v}$}{volume fraction}
\nomenclature[gWm]{$\phi_\mathrm{m}$}{mass fraction}
\nomenclature[gVf]{$\Upsilon_\mathrm{f}$}{fluid domain without particle surroundings}
\nomenclature[gepss]{$\bar{\varepsilon}$}{integral dissipation rate of the background flow field}
\nomenclature[geps']{$\varepsilon'$}{integral particle-induced dissipation rate}
...
\nomenclature[rn3]{$\vec{n}$}{normal vector}
...
\nomenclature[rE1]{$E_\mathrm{k}$}{turbulent kinetic energy of the fluid}
\nomenclature[rFp]{$\vec{F}_\mathrm{p}$}{force vector acting on the particles}
\nomenclature[rT1]{$\vec{T}_\mathrm{p}$}{particle torque vector}
\nomenclature[rE2]{$E_\mathrm{kB}$}{mean kinetic energy of the particles}
\nomenclature[rt3]{$t^\mathrm{*}$}{time normalized by initial eddy turnover time}
\nomenclature[ru4]{$\vec{u}_\mathrm{p}$}{vector of the undisturbed fluid velocity at the particle position}
\nomenclature[rV]{$V$}{control volume}
\nomenclature[rG(r)]{$G(r)$}{homogeneous filter function}
\nomenclature[ru2]{$\vec{u}'$}{vector of fluid velocity fluctuations}
\nomenclature[rmp]{$m_\mathrm{p}$}{mass of one particle}
\nomenclature[rmv]{$m_\mathrm{v}$}{fluid mass of one cell}
...
\begin{figure}[hb]
    ...
    \begin{minipage}[t]{0.5\textwidth}
	   ...
	   \caption{Temporal development of the kinetic energy $E_\mathrm{k}$ normalized by its initial value $E_\mathrm{k,0}$. Shortly after the injection, the PP-cases separate from the sP-flow. The higher-clustered cases show significant differences compared to the unclustered reference case}
...
\begin{figure}[hb]
    \centering
     \begin{minipage}[t]{0.5\textwidth}
        ...
        \caption{Temporal development of the point-particle coupling rate $\Psi_\mathrm{pp}$ normalized by $\varepsilon_\mathrm{ref}$ with constant $\lambda_\mathrm{c}$ for various resolutions. A correlation between the resolution and the accuracy can be observed.}

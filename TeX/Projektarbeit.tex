...
\nomenclature[g]{$\matrix{\tau}$}{Stress tensor}
...
\nomenclature[g]{$\gamma$}{Isentropic exponent}
...
\nomenclature[g]{$\eta$}{Kolmogorov length scale}
\nomenclature[g]{$\tau_\mathrm{\eta}$}{Kolmogorov time scale}
\begin{equation}
\eta = \biggl (\frac{\nu^3 L}{U^3} \biggl )^{1/4},
\end{equation}
\begin{equation}
...
\nomenclature[g]{$\lambda$}{Taylor microscale}
...
\newline
This work deals with particle laden fluids, therefore the impact of them on the flow behavior of the fluid needs to be described. The phenomenon described script is called two-way-coupling. 
\newline
In this context small and heavy rigid particles with a spherical shape are considered. Their radius $ r_p $ is even smaller than the Kolmogorov scale $ \eta $, but also large enough to neglect the Brownian motion.
\newline
Due to the \textbf{small particle concentration}, the best and most common way to describe these flows is the point particle approach, which means that every particle is treated as a mathematical point source of mass, momentum and energy. In this case we focus on the momentum exchange, effects like particle-particle interactions and particle-wall interactions are neglected.  
\newline
...
\newline
First the perspective is taken on the influence of the particles on the carrier fluid. Here the assumptions that the fluid is incompressible and that there is no mass exchange over the particle surface are used.
...
\nomenclature{$\vec{F}$}{Force per unit volume from particle on fluid}
The influence of the particles on the fluid is represented by the new term $\vec{F}$, which describes the force per unit volume on the fluid.
...
\begin{equation}
\mathrm{v}_\mathrm{p} \rho_\mathrm{p} \frac{\partial{\vec{v}_\mathrm{p}^\mathrm{n}}}{\partial{t}} = \mathrm{v}_\mathrm{p} \rho_\mathrm{p}\vec{g}+\vec{f}^\mathrm{n}(\vec{x}_\mathrm{p}^\mathrm{n})
\nomenclature[rg]{$\vec{g}$}{Gravity constant}
\end{equation}
...
m_\mathrm{p} \frac{\partial{\vec{v}_\mathrm{p}^\mathrm{n}}}{\partial{t}} = m_\mathrm{p}\vec{g}+\rho \mathrm{v}_\mathrm{p}\biggl(\frac{D\vec{u}}{Dt}-\vec{g} \biggl)+\vec{f}_\mathrm{d}+\vec{f}_\mathrm{l}+\vec{f}_\mathrm{a}+\vec{f}_\mathrm{h}+\vec{f}_\mathrm{additional}
\end{equation}
...
\begin{equation}
\vec{f}_\mathrm{d}=-\frac{3}{4}\rho \mathrm{v}_\mathrm{p} \frac{C_\mathrm{d}}{d}|\vec{v}_\mathrm{p}-\vec{u}|(\vec{v}_\mathrm{p}-\vec{u})
\mathrm{v} \rho_\mathrm{p} \frac{\mathrm{d}\vec{v}_\mathrm{p}^\mathrm{n}}{\mathrm{d}t} = \mathrm{v} \rho_\mathrm{p}\vec{g}+\vec{f}^\mathrm{n}(\vec{x}_\mathrm{p}^\mathrm{n}).
\end{equation}
\nomenclature{$\vec{g}$}{Gravity constant}
\nomenclature{$\mathrm{v}$}{Particle volume}
As already mentioned $\vec{f}^\mathrm{n}(\vec{x}_\mathrm{p}^\mathrm{n})$ could be divided in several forces. Then the equation of particle motion looks like
\begin{equation} \label{navier_stokes_particle}
m_\mathrm{p} \frac{\mathrm{d}\vec{v}_\mathrm{p}^\mathrm{n}}{\mathrm{d}t} = m_\mathrm{p}\vec{g}+\rho \mathrm{v}\biggl(\frac{\mathrm{D}\vec{u}}{\mathrm{D}t}-\vec{g} \biggl)+\vec{f}_\mathrm{d}+\vec{f}_\mathrm{l}+\vec{f}_\mathrm{a}+\vec{f}_\mathrm{h}+\vec{f}_\mathrm{additional}.
\end{equation}
Here $\vec{f}_\mathrm{d}$ represents the hydrodynamical drag force that is parallel to the undisturbed streamlines:
\begin{equation}
\vec{f}_\mathrm{d}=-\frac{3}{4}\rho \mathrm{v} \frac{C_\mathrm{d}}{d_\mathrm{p}}|\vec{v}_\mathrm{p}-\vec{u}|(\vec{v}_\mathrm{p}-\vec{u}).
\end{equation}
...
\begin{equation}
\vec{f}_\mathrm{a} =  \frac{1}{2}\rho \mathrm{v}_\mathrm{p} \biggl(\frac{D\vec{u}}{Dt}-\frac{d\vec{v}_\mathrm{p}}{dt}\biggl)
\vec{f}_\mathrm{a} =  \frac{1}{2}\rho \mathrm{v} \biggl(\frac{\mathrm{D}\vec{u}}{\mathrm{D}t}-\frac{\mathrm{d}\vec{v}_\mathrm{p}}{\mathrm{d}t}\biggl).
\end{equation}
...
\newline
In the following the fact that the added mass and history forces are negligible small compared with the drag force is shown using a rough approximation.
We use the approach that the relative velocity is of the same order as the terminal velocity $v_\mathrm{t} = \sqrt{(4/3)(\rho_\mathrm{p}/\rho)d_\mathrm{p}g/C_\mathrm{d}}$. The terminal velocity is the velocity of a particle in a resting fluid, when gravitation and drag force are in equilibrium. Then if the added mass and the drag force are compared, we get the following equation:
\begin{equation}
\frac{|\vec{f}_\mathrm{a}|}{|\vec{f}_\mathrm{d}} \simeq \frac{\frac{1}{2} \rho \mathrm{v}_\mathrm{p} g}{\frac{3}{4} \rho \mathrm{v}_\mathrm{p}(C_\mathrm{d}/d) v_\mathrm{t}^\mathrm{2}}\frac{a_\mathrm{r}}{g} = \frac{1}{2}\frac{\rho}{\rho_\mathrm{p}}\frac{a_\mathrm{r}}{g}
\frac{|\vec{f}_\mathrm{a}|}{|\vec{f}_\mathrm{d}|} \simeq \frac{\frac{1}{2} \rho \mathrm{v} g}{\frac{3}{4} \rho \mathrm{v}(C_\mathrm{d}/d_\mathrm{p}) v_\mathrm{t}^\mathrm{2}}\frac{a_\mathrm{r}}{g} = \frac{1}{2}\frac{\rho}{\rho_\mathrm{p}}\frac{a_\mathrm{r}}{g}
\end{equation}
with 
\begin{equation}
a_\mathrm{r}=(\frac{\mathrm{D}\vec{u}}{\mathrm{d}t}-\frac{\mathrm{d}\vec{v}_\mathrm{p}}{\mathrm{d}t}).
\end{equation}
\nomenclature[rar]{$a_\mathrm{r}$}{Relative acceleration of particles}
Since in gas-solid suspensions the particle density is by factor of 1000 higher than the fluid density the added mass force is negligible compared to the drag force if it is not the unlikely case that the relative acceleration is of the same order higher than the gravity.
...
\nomenclature[g]{$\tau_\mathrm{p}$}{Particle relaxation time}
...
\begin{equation}
\rho_\mathrm{p}\frac{\mathrm{d}\vec{v}_\mathrm{p}^\mathrm{n}}{\mathrm{d}t} = \rho_\mathrm{p}\vec{g}+\vec{f}_\mathrm{d}^\mathrm{n}.
\end{equation}
...
\begin{equation}
\rho_\mathrm{p}\frac{\mathrm{d}\vec{v}^\mathrm{n}}{\mathrm{d}t} = \rho_\mathrm{p}\vec{g}-\rho_\mathrm{p}\frac{\vec{v}_\mathrm{p}^\mathrm{n}-\vec{u}^\mathrm{n}}{\tau_\mathrm{p}^\mathrm{n}}.
\end{equation}
...
\nomenclature[g]{$\tau_\mathrm{ps}$}{Stokes relaxation time}
Using the Stokes response time $\tau_\mathrm{ps} =\frac{d_\mathrm{p}^2}{18 \nu} \frac{\rho_\mathrm{p}}{\rho}$, that is the particle response time at resting fluid conditions, there finally exists another form of this equation:
\nomenclature{$\tau_\mathrm{ps}$}{Stokes relaxation time}
\begin{equation}
\rho_\mathrm{p}\frac{\mathrm{d}\vec{v}_\mathrm{p}^\mathrm{n}}{\mathrm{d}t} = \rho_\mathrm{p}\vec{g}-\rho_\mathrm{p}\frac{\vec{v}_\mathrm{p}^\mathrm{n}-\vec{u}^\mathrm{n}}{\tau_\mathrm{ps}}f_\mathrm{d},
\end{equation}
...
\nomenclature[g]{$\rho_\mathrm{p}$}{Particle density}
...
\psi = \vec{F} \vec{u}. 
\end{equation}
\nomenclature[g]{$\psi$}{Coupling rate}

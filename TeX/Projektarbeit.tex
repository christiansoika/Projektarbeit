...
\vec{\nabla} \cdot\vec{u}.
\end{equation}
The Navier-Stokes equations, described in chapter 1, become:
\begin{equation}
...
Due to this assumptions velocity equation (3.17) becomes
\begin{equation}
\rho_\mathrm{p}\frac{d\vec{v_\mathrm{p}}^\mathrm{n}}{dt} = \rho_\mathrm{p}\vec{g}+\vec{f}_\mathrm{d}^\mathrm{n}
\end{equation}
This equation can be rewritten with help of the relaxation time $\tau_\mathrm{p}$, which physically represents the time scale over which the drag force decreases the particle relative velocity to zero. At one point in the fluid with the proportionality 
\begin{equation}
...
With equation (3.26) and the Reynolds number in relation to the particles $Re_\mathrm{p}^\mathrm{n}=\frac{v_\mathrm{r}^\mathrm{n}d}{\nu}$, with the particle diameter d it follows
\begin{equation}
...
\end{equation}
Using the Stokes response time $\tau_\mathrm{ps} = \frac{\rho_\mathrm{p}d^2}{18\nu\rho}$, that is the particle response time at resting fluid conditions, there finally exists another form of this equation 
